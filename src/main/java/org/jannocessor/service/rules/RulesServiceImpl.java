...
public class RulesServiceImpl implements RuleExecutor, Settings {
	...
	public RulesServiceImpl(Configuratîr config, PathLocator paths) {
		this.config = config;
		this.paths = paths;
	}

	public final void executeRules(List<Object> facts,
			Map<String, Object> globals) throws JannocessorException {
		logger.info("Executing rules...");

		try {
			execute(facts, globals);
		} catch (Exception e) {
	...
	private final void execute(List<?> facts, Map<String, Object> globals)
			throws Exception {
		...
		KnowledgeBase kbase;
		if (isRecompilationRequired(resources)) {
